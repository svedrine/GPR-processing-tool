...
for rx in range(1, nrx + 1):
	...
	if args.gain:
		if len(args.gain) != 2:
			raise CmdInputError('args.gain shape must be (type, inittime)')
		else:
			...
			for output in availableoutputs:
				# Initialize
				grp.create_dataset(output, (iterations,), dtype=f[path + output].dtype)
				fout[path + output][:,] = f[path + output][:,]
				# Apply gain
				fout[path + output][tindex:,] *= fgain[tindex:]
	...
	if len(args.outputs) == 1:
		...
		if args.gain:
			outputdata = fout[path + output][:] * polarity
		else: 
