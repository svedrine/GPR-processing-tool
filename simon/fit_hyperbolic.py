...
parser.add_argument('-p', type=float, help='x0 (m), t0 (ns), 0.3 > v(m/ns) > 0.03, R (m) radius', nargs=4)
...
for rx in range(1, nrx + 1):
	...
	plt.imshow(outputdata, extent=[amin(xcoordonates), amax(xcoordonates), time, 0], interpolation='nearest', aspect='auto', cmap='seismic', vmin=-amax(abs(outputdata)), vmax=amax(abs(outputdata)))
	plt.plot(x, fit, color='g', linewidth=2.0)
